...
CREATE TABLE `test_report_template_record`  (
...
CREATE TABLE `test_original_record_template_record`  (
...
--工时统计
--整理旧任务数据
    INSERT INTO test_task_statistics (
	id,
	CODE,
	entrustment_id,
	orderer,
  ...
  FROM
          test_task t1
              LEFT JOIN test_report_record t2 ON t1.entrustment_id = t2.entrustment_id
  WHERE
              t2.state >= 6

--整理旧检测项数据
      INSERT INTO test_entrusted_sample_checkitem_statistics (
    id,
    entrust_id,
    sample_id,
    check_item_name,
