...
public class TestInstrumentTypeController extends ApiController {
    ...
    @GetMapping("/getList")
    public Result getAll(TestInstrumentType testInstrumentType) {
        QueryWrapper<TestInstrumentType> queryWrapper=new QueryWrapper<>(testInstrumentType);
        queryWrapper.orderByDesc("create_time");
        queryWrapper.eq("del_flag",0);
        return ResultUtil.success(this.testInstrumentTypeService.list(queryWrapper));
    }
    /**
     * 分页查询所有数据
     *
     ...
     * @return 新增结果
     */
    @Log(title = "新增仪器分类", businessType = BusinessType.INSERT)
    @PostMapping("/add")
     ...
     * @return 修改结果
     */
    @Log(title = "编辑仪器分类", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
     ...
     * @return 删除结果
     */
    @Log(title = "删除仪器分类", businessType = BusinessType.DELETE)
    @PostMapping("/del")
